...
impl SpaPacket {
    ...
    /// `transport` selects the ephemeral wire encoding (raw point vs Elligator2
    /// representative, §14) — the inner record and signature are identical either
    /// way.
    pub fn build(
    ...
    // The SPA inner record genuinely has this many independently-signed fields;
    // bundling them into a struct would just move the argument list elsewhere.
    ...
    pub fn build_with_padding(
        ...
        // Mint the ephemeral first so the signature can commit to its *point*,
        // then seal the inner record under that same ephemeral. For the
        // obfuscated transport the wire carries the Elligator2 representative,
        // but the signature (like the seal KDF/AAD) binds the recovered point.
        let (ephemeral_secret, ephemeral_pub, representative) = match transport {
            SealTransport::Raw => {
                let secret = seal::generate_ephemeral();
                let point = seal::ephemeral_public(&secret);
                (secret, point, None)
            }
            SealTransport::Obfuscated => {
                let (secret, representative, point) = seal::generate_obfuscated_ephemeral();
                (secret, point, Some(representative))
            }
        };
        ...
        let mut wire = match representative {
            None => seal::seal_with_ephemeral(&inner, server_static_pubkey, &ephemeral_secret),
            Some(representative) => seal::seal_with_ephemeral_obfuscated(
                &inner,
                server_static_pubkey,
                &ephemeral_secret,
                &representative,
    ...
    /// parse the inner record. `transport` must match the encoding the client
    /// used (operator-configured per deployment, §9). Does **not** verify the
    /// signature — call [`verify`] after looking up the client key by `key_id`.
    pub fn open(
        wire: &[u8],
        ...
        let opened = match transport {
            SealTransport::Raw => seal::open(sealed_core, server_static_private),
            SealTransport::Obfuscated => seal::open_obfuscated(sealed_core, server_static_private),
        }
        .ok_or(SpaError::SealOpenFailed)?;
...
mod tests {
    ...
    fn builds_opens_and_verifies_udp_packet() {
        ...
        let packet = SpaPacket::open(&wire, &SERVER_PRIV_A, SealTransport::Raw).expect("opens");
    ...
    fn packet_built_for_one_server_does_not_open_on_another() {
        ...
        assert_eq!(
            ...
            SpaPacket::open(&wire, &SERVER_PRIV_B, SealTransport::Raw)
        );
    }
    ...
    #[test]
    fn obfuscated_transport_builds_opens_and_verifies() {
        // The obfuscated transport round-trips and the recovered ephemeral point
        // still verifies the inner signature (the signature binds the point, not
        // the wire representative).
        let signing_key = generate_ed25519_keypair();
        let server_public = server_pub(&SERVER_PRIV_A);
        let wire = SpaPacket::build(
            &signing_key,
        ...
        let packet = SpaPacket::open(&wire, &SERVER_PRIV_A, SealTransport::Obfuscated)
            .expect("opens with the matching transport");
        assert_eq!(42, packet.counter);
        assert_eq!(Ipv4Addr::new(203, 0, 113, 7), packet.allow_ip);
        packet
            .verify(&signing_key.verifying_key(), &server_public)
            .expect("signature verifies over the recovered point");
    }

    #[test]
    fn transport_mismatch_fails_to_open() {
        // A verifier configured for a single transport rejects the other: opening
        // an obfuscated packet as raw treats the representative as a curve point
        // (wrong DH input → AEAD fails), and vice-versa.
        let signing_key = generate_ed25519_keypair();
        let server_public = server_pub(&SERVER_PRIV_A);

        let obfuscated = SpaPacket::build(
            &signing_key,
            SpaMode::Udp,
            1_000_000,
            1,
        ...
        assert_eq!(
            Err(SpaError::SealOpenFailed),
            SpaPacket::open(&obfuscated, &SERVER_PRIV_A, SealTransport::Raw)
        );

        let raw = SpaPacket::build(
            &signing_key,
            SpaMode::Udp,
            1_000_000,
            1,
        ...
        assert_eq!(
            Err(SpaError::SealOpenFailed),
            SpaPacket::open(&raw, &SERVER_PRIV_A, SealTransport::Obfuscated)
        );
    }
}
