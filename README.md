...
The command also prints an `authorized_keys.toml` entry. Add both public keys to the server; the server verifies SPA signatures with `public_key` and verifies the Noise XK remote static key with `noise_public_key`.
...
Example `/etc/ghostbro/ghostbro.toml`:
...
time_window = 300
allow_ttl = 14400
counter_state = "/var/lib/ghostbro/spa-counters.toml"
...
[logging]
path = "/var/log/ghostbro/server.log"
level = "info"
```
...
sudo RUST_LOG=info target/debug/ghostbro-server \
  --config /etc/ghostbro/ghostbro.toml \
  --iface eth0 \
...
capabilities. The shipped systemd unit `deploy/ghostbro.service` runs the
daemon as the unprivileged `ghostbro` user with exactly
...
to `/var/lib/ghostbro` + `/var/log/ghostbro`.
...
```bash
sudo useradd --system --no-create-home --shell /usr/sbin/nologin ghostbro
sudo install -Dm755 target/release/ghostbro-server /usr/local/bin/ghostbro-server
sudo install -Dm644 deploy/ghostbro.service /etc/systemd/system/ghostbro.service
sudo systemctl daemon-reload && sudo systemctl enable --now ghostbro
```
...
Start the local SOCKS5 listener and let the client send SPA, establish Noise XK, and fail over across configured servers if needed.
...
Relay results are stored per client key ID under `GHOSTBRO_RELAY_SPOOL_DIR` or `/var/lib/ghostbro/relay` by default. Loopback/private URL fetches are blocked unless `GHOSTBRO_RELAY_ALLOW_LOOPBACK=1` is set for local smoke testing.
...
- **Server hardening (§13.4)** — the daemon runs unprivileged via the shipped `deploy/ghostbro.service` unit (User= + ambient `CAP_BPF`/`CAP_NET_ADMIN`/`CAP_NET_BIND_SERVICE`) and reports its privilege level at startup. See [§3a](#3a-run-the-server-unprivileged-production-hardening).

### Future Work (explicitly deferred in PRD §14)

- **SOCKS5 UDP ASSOCIATE** — current SOCKS5 support is no-auth TCP CONNECT over Noise XK.
